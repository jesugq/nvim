do
  ...
  vim.keymap.set('n', '<leader>s"', function() Snacks.picker.registers({ transform = function(item) return nevim_marks(item) end }) end, { desc = 'Snacks picker numbered/default registers' })
  vim.keymap.set('n', '<leader>sq', function() Snacks.picker.registers({ transform = function(item) return human_marks(item) end }) end, { desc = 'Snacks picker macro registers' })
